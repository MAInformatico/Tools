...
sudo touch today$(date +%d%m%y).pcap && sudo tshark -nni <your device i.e.: eth0> -a duration:<time in seconds, i.e: 86400>  -w today$(date +%d%m%y).pcap && tar -cvf compress.tar today$(date +%d%m%y).pcap
